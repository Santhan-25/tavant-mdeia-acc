...
def get_ad_data():
    ...
    for row in ad_data:
        row_data = {
            ...
            'budget': f"{math.ceil(row['bookedb']):,}",
            'revenue_risk': f"{math.ceil(row['rar']):,}",
            'total_campaigns': f"{math.ceil(row['total_campaigns']):,}",
            'pending_review': f"{math.ceil(row['pr_count']):,}",
            'ready_for_invoice': f"{math.ceil(row['rfi_count']):,}",
        }
...
gb.configure_grid_options(rowHeight=65, tooltipShowDelay=0, tooltipHideDelay=2000, cellStyle={'text-align': 'center'},configure_selectable=True)
...
#grid_options['paginationPageSize'] = 15
...
grid_response  = AgGrid(filtered_data, gridOptions=grid_options, enable_enterprise_modules=True, theme='light', height=750, fit_columns_on_grid_load=True,
...
if grid_response['selected_rows'] is not None:
    ...
    if selected_rows is not None and not selected_rows.empty:
            st.session_state["selected_advertiser"] = selected_rows
